...
import { deleteVideo } from '../Redux/DeleteReducer/action'
import { editVideo } from '../Redux/updateReducer/action';
...
class Card extends Component {
  ...
  generateEncryptedId(originalVideoId) {
    const encryptedVideoId = CryptoJS.AES.encrypt(originalVideoId, 'BBB_secretD_key').toString();
      ...
      <div>
        <div className="card">
          <div className='video-container'>
            <iframe src={element.video} frameborder="0" allowfullscreen title={"cardVideo"}></iframe>
          </div>
          <div className='d-cnt'>
            <h3 onClick={this.toggleTitle}>{displayTitle}<span id="show">{showFulltitle ? 'Show Less' : 'Show More...'}</span></h3>
            ...
            <Link to={`/overview/${encryptedVideoId}`} className="card-link"><p>overview</p></Link>
            <div id='DAE'>
              <button className="edit" onClick={() => { this.openForm(element) }} ><LuEdit />EDIT</button>
              <button className="Delete" onClick={() => { this.openBox() }} ><MdOutlineDeleteOutline />DELETE</button>
            </div>
          </div>
        </div>
        ...
        <Dialog className='Editbox' open={showEditConfirm} onClose={this.closeForm} style={{margin:'auto'}} >
          ...
          <DialogActions style={{justifyContent: 'space-between'}}>
            <Button onClick={this.closeForm} style={{color:'red'}}>Cancel</Button>
...
const mapDispatchToProps = (dispatch) => {
  return {
    deleteVideo: (obj) => { dispatch(deleteVideo(obj)) },
    editVideo: (obj) => { dispatch(editVideo(obj)) },
